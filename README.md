...
Note for any future teams/development. You will first need to run `npm install` to download all the needed packages. All JavaScript files are also bundled. To make any further additions or changes, edit the JavaScript in `app.js` or `model.js`, and then run the build command found in `package.json`.
